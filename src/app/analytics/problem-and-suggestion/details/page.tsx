...
const iconMap: Record<string, ReactElement> = {
  ...
  Lightbulb: <Lightbulb className="h-8 w-8 mb-4 text-primary" />,
  TrendingDown: <TrendingDown className="h-8 w-8 mb-4 text-primary" />,
};
    ...
    <div className="flex-1 container mx-auto px-4 py-8 sm:px-6 lg:px-8 flex flex-col">
      ...
      {isLoading ? (
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[...Array(3)].map((_, i) => (
            <Card key={i} className="flex flex-col p-4 w-full bg-card/60 backdrop-blur-sm">
              <Skeleton className="h-8 w-8 mb-4" />
              <CardHeader className="p-0">
                <Skeleton className="h-6 w-3/4" />
              </CardHeader>
              <CardContent className="p-0 mt-2 flex-1 space-y-2">
